...
let rec fmt1 (e : expr) : string = 
    match e with
      ...
      | If(con,is,not) -> String.concat " " ["if"; fmt1 con; "then"; fmt1 is; "else"; fmt1 not]
...
let rec fmt2 (ctxpre : int) (e : expr) = 
    match e with
      ...
      | _ -> raise (Failure "reached an unidentified token")
...
let rec eval (e : expr) (env : (string * int) list) : int =
    match e with
      ...
      | _                 -> raise (Failure "reached an unknown token")
...
let rec closedin (e : expr) (env : string list) : bool =
    match e with 
      ...
      | _                 -> raise (Failure "cannot infer expr")
...
let rec freevars e : string list =
    match e with 
      ...
      | _                 -> raise (Failure "cannot infer item")
...
let rec tcomp e (cenv : string list) : texpr =
    match e with
      ...
      | _ -> raise (Failure "Cannot compile expr to texpr")
...
let rec scomp e (cenv : rtvalue list) : sinstr list =
    match e with
      ...
      | _ -> raise (Failure " scomp: Cannot read expr")
