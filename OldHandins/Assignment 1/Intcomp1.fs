...
module Intcomp1
    ...
    let rec eval e (env : (string * int) list) : int =
        ...
        | Var x             -> lookup env x 
        | Let((x,y)::xs, ebody) -> 
        let env1 = (x,(eval y env)) :: env 
        eval ebody env1
        | Let([], ebody) -> eval ebody []
    ...
    let rec closedin (e : expr) (vs : string list) : bool =
        ...
        | Let((x,y)::xs, ebody) ->
        let vs1 = x :: vs
        closedin y vs && closedin ebody vs1
    ...
    let rec nsubst (e : expr) (env : (string * expr) list) : expr =
        ...
        Let([x,y], nsubst ebody newenv) // ?? hmm
    ...
    let rec freevars e : string list =
        ...
        | Var x  -> [x]
        | Let((x,y)::xs, ebody) ->
            union (freevars y, minus (freevars ebody, [x])) // Something about freevars and the list ????? TODO::::
    ...
    let rec tcomp (e : expr) (cenv : string list) : texpr =
        ...
        | Let((x,y)::xs, ebody) -> 
        let cenv1 = (x,y) :: cenv 
        TLet(tcomp y cenv, tcomp ebody cenv1)
